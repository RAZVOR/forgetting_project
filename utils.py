...
def single_epoch0(model, loader, opt, loss_fn, scheduler=None):
...
def single_epoch(model, loader, opt, loss_fn, scheduler=None):
    ...
    mask_after_opt = torch.zeros(len(loader.dataset))
    conf_after_opt = torch.zeros(len(loader.dataset))
    latent_space = torch.zeros(len(loader.dataset), 128)
    ...
    for ims, labs, ids in loader:
        ...
        with autocast():
            latent = model.get_latent(ims)
            out = model.get_out(latent)
            loss = loss_fn(out, labs)
            total_loss += loss.cpu().item()
            ...
            mask_after_opt[ids.squeeze(-1)] = correct_mask.float().cpu()
            conf_after_opt[ids.squeeze(-1)] = conf_mask.float().cpu().clone().detach()
            latent_space[ids.squeeze(-1)] = latent.float().cpu().clone().detach()
            probs[ids.squeeze(-1)] = torch.softmax(out, axis=1).float().cpu().clone().detach()
    ...
    train_ret["accuracy"], train_ret["loss"], train_ret["acc_mask"], train_ret["conf_mask"], train_ret["latent_embedding"], train_ret["probs"] = acc, loss, mask_after_opt, conf_after_opt, latent_space, probs
    
    return train_ret


def train_ssf(model, loader, opt, scheduler, loss_fn, EPOCHS, patience = 5, eval_every = False, eval_loader= None, test_loader = None):
    ...
    for ep in range(EPOCHS):
        ...
        if test_loader is not None:
            test_ret = eval(model, test_loader, eval_mode = True)
            latent_emb_test_list.append(test_ret["latent_emb"])
    ...
    return_dict["latent_emb"] = latent_emb_list
    return_dict["latent_emb_test"] = latent_emb_test_list
    return_dict["probs"] = probs_list
...
def eval(model, loader, eval_mode = True):
    ...
    latent_emb = torch.zeros(len(loader.dataset), 128)
    with torch.no_grad():
        ...
        for ims, labs, ids in loader:
            ...
            with autocast():
                ...
                latent_emb[ids.squeeze(-1)] = latent.float().cpu().detach()
