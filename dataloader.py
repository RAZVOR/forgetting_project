...
def return_loaders(all_args, get_frac = True):
    ...
    preloader = DataLoader(dataset=d1_tr, batch_size=batch_size, shuffle=True, num_workers=8)
    ftloader = DataLoader(dataset=d2_tr, batch_size=batch_size, shuffle=True, num_workers=8)
    ...
    preloader_test = DataLoader(dataset=d1, batch_size=batch_size, shuffle=False, num_workers=8)
    ftloader_test = DataLoader(dataset=d2, batch_size=batch_size, shuffle=False, num_workers=8)
...
def dataset_with_indices0(cls):
    ...
    #return type('torchvision.datasets.mnist.MNIST', (cls,), {
        '__getitem__': __getitem__,
    })

class MNIST(datasets.mnist.MNIST):
    def __getitem__(self, index):
